...
<div class="modal-edit" style="width: 450px;">
    ...
    <?php if (isset($_GET["edit"])) : ?>
        ...
        $news = $this->news_service->getById($_GET["edit"])["data"];

        ?>

        <h1 style="padding: 4px 0 14px 0;">Edit berita</h1>
        <form action="/api/news/update" method="post" key="<?= time() ?>">
            <input type="hidden" name="id" value="<?= $news["id"] ?>">

            <div class="field">
                <label for="">Head</label>
                <input required type="text" name="head" value="<?= $news["head"] ?>">
            </div>
            <div class="field">
                <label for="">Sub head</label>
                <input required type="text" name="sub_head" value="<?= $news["sub_head"] ?>">
            </div>
            <div class="field">
                <label for="">Content</label>
                <textarea name="content" rows="6" id=""><?= $news["content"] ?></textarea>
            </div>
            <div class="field">
                <label for="">Tanggal</label>
                <input required type="date" name="dibuat" value="<?= $news["dibuat"] ?>">
            </div>
