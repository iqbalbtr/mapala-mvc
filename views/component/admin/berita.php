...
<div style="width: 100%; display: flex; justify-content: space-between; padding: 8px 12px; align-items: center;">
    <h1>Daftar Berita</h1>
    <a href="?page=berita&aksi=tambah" class="add-btn">
        Tambah
...
<div>
    <table>
        ...
        <tbody>
            <?php 
            $i = $newses["page"] > 1 ? ($newses["page"] - 1) * 5 + 1 : $newses["page"] ; 
            ?>
            <?php foreach ($newses["data"] as $news) : ?>
                <tr>
                    <td><?= $i ?></td>
                    <td><?= $news["head"] ?></td>
                    <td><?= $news["sub_head"] ?></td>
                    <td><?= $news["content"] ?></td>
                    <td class="aksi">
                        <a class="delete-btn" href="/api/news/delete?id=<?= $news["id"] ?>">
                        ...
                        <a class="edit-btn" href="?page=berita&edit=<?= $news["id"] ?>">
    ...
    <div class="pagination">
        <?php for ($i = 1; $i <= $newses["pagination"]; $i++) : ?>
            <a href="/admin?page=berita&offset=<?= $i ?>" class="pagination-btn"><?= $i ?></a>
        <?php endfor ?>
    </div>
</div>